...
def check_imgsz(imgsz, stride=32, min_dim=1, max_dim=2, floor=0):
    ...
    stride_tensor = torch.cat(stride).to(torch.int) if isinstance(stride, (list, tuple)) and all(isinstance(s, torch.Tensor) for s in stride) else torch.tensor([min_dim])
    stride = int(stride_tensor.max().item()) if stride_tensor.numel() > 0 else min_dim
