...
class BasePredictor:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.TensorRt = self.args.TensorRt
    ...
    def pre_transform(self, im):
        ...
        # auto = same_shapes
        return [LetterBox(self.imgsz, auto=auto, stride=self.model.stride, TensorRt = self.TensorRt)(image=x) for x in im]
            ...
            if isinstance(results,list):
                ...
                if self.args.save or self.args.show:  # Add bbox to image
                    ...
                    if not self.args.retina_masks:
                        ...
                        img, boxes_list = result.plot(**plot_args)
                        plotted_img.append(img)
            else:
    ...
    def __call__(self, source=None, model=None, stream=False):
        ...
        if stream:
            ...
            return self.stream_inference(source, model) # merge list of Result into one
    ...
    def stream_inference(self, source=None, model=None):
        ...
        for batch in self.dataset:
            ...
            with profilers[2]:
                ...
                for i, pred in enumerate(preds):
                    ...
                    elif pred.ndim == 3:  # i = 0 => det
                        pred = self.postprocess_det(pred, im, im0s)
                        self.results.append(pred)
                    elif pred.ndim == 4:       # i = 1, 2 => seg
            ...
            for i in range(n):
                ...
                if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                    ls, boxes_list = self.write_results(i, self.results, (p, im, im0))
                    s += ls
        ...
        if isinstance(self.plotted_img[2], np.ndarray):
            self.plotted_img = self.plotted_img[::-1]

        return boxes_list, self.plotted_img
    ...
    def show(self, p):
        ...
        array_idx = [i for i, x in enumerate(im0) if isinstance(x, np.ndarray)][0]
        if platform.system() == 'Linux' and p not in self.windows:
            self.windows.append(p)
            cv2.namedWindow(str(p), cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)  # allow window resize (Linux)
            cv2.resizeWindow(str(p), im0[array_idx].shape[1], im0[array_idx].shape[0])        
        
        cv2.imshow(str(p), im0[array_idx])
