    ...
    def plot(
        ...
        if pred_boxes and show_boxes:
            for d in reversed(pred_boxes):
                ...
                #
                boxes = d.xyxy[0].cpu().numpy()
                id = int(d.cls.cpu().numpy())
                cof = d.conf.cpu().numpy()
                combined = np.concatenate((boxes, id,cof), axis=None)
                boxes_list.append(combined)
                #
        ...
        return annotator.result(), boxes_list
